    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        if order_exists:
            ...
            try:
                ...
                if order:
                    order.delete()
                return HttpResponse(content=f' Unhandled webhook received: {event["type"]} | ERROR: {e}', status=500)

        return HttpResponse(
            content=f' Unhandled webhook received: {event["type"]} || SUCCESS: Created order in webhook',
            status=200
        )
